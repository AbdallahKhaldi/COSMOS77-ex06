"""The inter-group BONUS series (E12) for the live web console — kept out of match.py.

Runs the 6-sub-game role-swap over four cross-group MCP URLs, streams it live via the
``on_event`` hook, and writes the byte-identical ``bonus_game`` JSON. The shared token +
the four URLs are runtime-only (never written to config; Rule 9). No LLM here.
"""
