...
| R5 | **Bonus JSON mismatch** — the two groups' `bonus_game` reports differ byte-for-byte → both score **0**. | Low / High | Both groups serialize through the **same canonical serializer** (sorted keys, fixed formatting) shared from `report/builder.py`; agree config + the four MCP URLs + a shared token up front; run a `diff` check on both JSONs **before** either group emails; set `mutual_agreement: true` only after the diff is clean. | E7, E12 |
