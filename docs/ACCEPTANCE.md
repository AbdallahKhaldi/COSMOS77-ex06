...
- **DONE:** E1, E2, E3, E4, E5, E6, E8, E9, E10, E11, E13 — **11** (E3/E4/E5/E6/E10 **live-validated** against real Gemini + public cloud URLs).
- **READY — awaits a human step:** **E7** (Gmail OAuth `credentials.json` + the one-time consent → `report --send`) and **E12** (needs a partner group) — **2**.
- **Net:** 11 DONE, 2 READY — only the Gmail send and the inter-group bonus remain, both gated purely on a human action.
