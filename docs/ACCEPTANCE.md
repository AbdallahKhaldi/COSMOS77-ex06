...
| **E5** | Fully autonomous pipeline: init → 6 valid sub-games → automated report, zero manual intervention | `orchestrator/runner.py` (`run_full_game`), `sdk/sdk.py`, `cli/main.py` (`run`), cop auto-emails via `report/dispatch.auto_send` | `tests/unit/test_game/test_game_runner.py`, `test_orchestrator/*`, `test_report/test_dispatch.py` (mocked agents → 6 valid sub-games + auto-send) | **DONE (live-validated)** — the headline **6-sub-game game ran autonomously** on `gemini-2.5-flash-lite` (billing on): 4×4, **cop 75 / thief 45** (3 captures + 3 full-survival evasions), zero manual steps, saved to `reports/internal_game.json`. (The end-of-game email is E7.) |
| **E6** | Local → cloud: proven on localhost (separate ports), then public HTTPS URLs (Horizon/tunnel) with token auth | `orchestrator/cloud.py` (HTTPS guard, `run_cloud_game`), `--cloud` flag, `deploy/horizon.md`, `deploy/tunnel.sh`, config `mcp.cop_url/thief_url` | `tests/unit/test_deploy/*` (cloud build, cross-process state sync, HTTPS guard) — all mocked, no network in CI | **DONE (live-validated)** — ran `--cloud` over **public Cloudflare-tunnel HTTPS URLs** with token auth (`auth=ok`); proof in `reports/cloud_run.log` (the two `*.trycloudflare.com/mcp` URLs in the per-turn comms). Cross-process `sync_state` held (the cop captured over the tunnels). Config reverts to `localhost` (tunnel URLs are ephemeral). |
| **E7** | Automated Gmail JSON report — cop auto-sends ONE email (JSON body only) to `rmisegal+uoh26b@gmail.com` at game end | `report/gmail_sender.py` (OAuth desktop, JSON-only body, base64url, `userId="me"`), `report/dispatch.py`, `report/builder`/`output.py`, `report/schema.py` (pydantic), config `report.to` | `tests/unit/test_report/*` (mocked `googleapiclient`; JSON-only body; `messages().send`; token refresh) — `report/` at **100%** | **DONE (live-validated)** — Gmail OAuth completed (`credentials.json` + `token.json`); a **real email was sent via the Gmail API** (gatekeeper record `sent: true`, JSON-only body). The professor receives it only via the guarded `report --send --final` (a bare send is refused). |
...
| **E10** | GUI showing the game in real time + CLI logs proving valid MCP comms | `gui/viewer.py`, `render.py`, `theme.py`, `driver.py` (headless-safe); structured per-turn logs via `orchestrator/turn_log.py` + `logging_setup.py` (`show_server_url: true`) | `tests/unit/test_gui/*` (render draw-calls, turn-log format incl. MCP URL + NL message); committed frames `assets/gui_start.png`, `gui_midgame.png`, `gui_capture.png` | **DONE** — GUI render + log format + screenshots (CI-proven); the **cloud-MCP CLI log is captured** in `reports/cloud_run.log` (real comms over the public `*.trycloudflare.com` URLs). |
...
- **DONE:** E1–E11 + E13 (everything except E12) — **12** (E3/E4/E5/E6/E7/E10 **live-validated**: real Gemini games, public cloud URLs, and a real Gmail send).
- **READY — awaits a partner group:** **E12** only (the inter-group bonus harness is code-complete + tested) — **1**.
- **Net:** 12 DONE, 1 READY. The professor receives the real report only via the guarded `report --send --final`.
